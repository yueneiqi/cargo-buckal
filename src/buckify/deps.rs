use std::{collections::{BTreeSet as Set, HashMap}, path::PathBuf};
...
fn resolve_first_party_label(dep_package: &Package) -> Result<String> {
    ...
    let manifest_dir = manifest_path.parent().context("manifest_path should always have a parent directory")?;
